...
Современное web-приложение для пакетного сжатия изображений прямо в браузере. Файлы никогда не покидают ваше устройство — вся обработка происходит локально через WebAssembly.
...
### Профессиональное сжатие (WASM)
Приложение использует библиотеки **MozJPEG**, **libwebp** и **AVIF** (через WebAssembly):
...
| **Сжатие** | Web Workers + WebAssembly (`@jsquash/jpeg`, `@jsquash/webp`, `@jsquash/avif`) |
